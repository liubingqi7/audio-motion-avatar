...
class Point(Dict):
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        if depth is None:
            ...
            grid_max = int(self.grid_coord.max())
            depth = grid_max.bit_length()
            # Ensure depth is at least 1 to avoid errors in hilbert encoding

        if depth < 1:
            print(f"depth is {depth}, grid_max is {grid_max}")
            print(f"self.grid_coord max is {self.grid_coord.max()}")
            print(f"self.grid_coord min is {self.grid_coord.min()}")
